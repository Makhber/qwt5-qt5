    ...
    : QObject(plot), d_selectedCurve(NULL), d_selectedPoint(-1)
{
    ...
    const char *text = "All points can be moved using the left mouse button "
                       "or with these keys:\n\n"
...
void CanvasPicker::moveBy(int dx, int dy)
{
    ...
    const int x =
            plot()->transform(d_selectedCurve->xAxis(), d_selectedCurve->x(d_selectedPoint)) + dx;
    const int y =
            plot()->transform(d_selectedCurve->yAxis(), d_selectedCurve->y(d_selectedPoint)) + dy;

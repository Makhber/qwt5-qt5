...
Plot::Plot(QWidget *parent) : QwtPlot(parent)
{
    ...
    connect(d_colorBar, SIGNAL(selected(const QColor &)), SLOT(setCanvasColor(const QColor &)));
    ...
    connect(d_wheel, SIGNAL(valueChanged(double)), SLOT(scrollLeftAxis(double)));

    // we need the resize events, to lay out the wheel
    canvas()->installEventFilter(this);

    d_colorBar->setWhatsThis("Selecting a color will change the background of the plot.");
    scaleWidget->setWhatsThis("Selecting a value at the scale will insert a new curve.");
    d_wheel->setWhatsThis("With the wheel you can move the visible area.");
    axisWidget(xBottom)->setWhatsThis("Selecting a value at the scale will insert a new curve.");
}
...
bool Plot::eventFilter(QObject *object, QEvent *e)
{
    if (e->type() == QEvent::Resize) {
        ...
        if (object == (QObject *)axisWidget(yLeft)) {
            ...
            const int h =
                    size.height() - scaleWidget->startBorderDist() - scaleWidget->endBorderDist();
        ...
        if (object == canvas()) {
            ...
            d_wheel->setGeometry(cr.right() - margin - w, cr.center().y() - h / 2, w, h);
        }
    }
...
void Plot::insertCurve(Qt::Orientation o, const QColor &c, double base)
{
    QwtPlotCurve *curve = new QwtPlotCurve();

    curve->setPen(c);
    curve->setSymbol(QwtSymbol(QwtSymbol::Ellipse, Qt::gray, c, QSize(8, 8)));

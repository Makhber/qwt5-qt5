...
{
public:
    Knob(const QString &title, double min, double max, QWidget *parent) : QWidget(parent)
    {
    ...
    double value() const { return d_knob->value(); }
...
protected:
    virtual void resizeEvent(QResizeEvent *e)
    {
        ...
        d_knob->setGeometry(0, d_label->pos().y() - h + off, sz.width(), h);
    }
...
{
public:
    Thermo(const QString &title, QWidget *parent) : QWidget(parent)
    {
...
AmpFrame::AmpFrame(QWidget *p) : QFrame(p)
{
...
void AmpFrame::timerEvent(QTimerEvent *)
{
    ...
    const double sig_bass = (1.0 + 0.1 * d_knbBass->value()) * sin(13.0 * phs);
    const double sig_mid_l = sin(17.0 * phs);
    const double sig_mid_r = cos(17.5 * phs);
    const double sig_trbl_l = 0.5 * (1.0 + 0.1 * d_knbTreble->value()) * sin(35.0 * phs);
    const double sig_trbl_r = 0.5 * (1.0 + 0.1 * d_knbTreble->value()) * sin(34.0 * phs);

    double sig_l =
            0.05 * d_master * d_knbVolume->value() * qwtSqr(sig_bass + sig_mid_l + sig_trbl_l);
    double sig_r =
            0.05 * d_master * d_knbVolume->value() * qwtSqr(sig_bass + sig_mid_r + sig_trbl_r);

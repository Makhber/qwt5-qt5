...
{
public:
    PrivateData() : options(QwtPlotPrintFilter::PrintAll), cache(NULL) { }

    ~PrivateData() { delete cache; }
...
void QwtPlotPrintFilter::apply(QwtPlot *plot) const
{
    ...
    for (int axis = 0; axis < QwtPlot::axisCnt; axis++) {
        ...
        if (scaleWidget) {
            cache.scaleColor[axis] =
                    scaleWidget->palette().color(QPalette::Active, Palette::WindowText);
            ...
            if (scaleTitle.testPaintAttribute(QwtText::PaintUsingTextColor)) {
                ...
                scaleTitle.setColor(color(cache.scaleTitleColor[axis], AxisTitle));
            }

            if (scaleTitle.testPaintAttribute(QwtText::PaintUsingTextFont)) {
                cache.scaleTitleFont[axis] = scaleTitle.font();
                scaleTitle.setFont(font(cache.scaleTitleFont[axis], AxisTitle));
            }
    ...
    if (hasBackgroundColor(plot)) {
        ...
        cache.widgetBackground = plot->palette().color(QPalette::Active, Palette::Window);
...
void QwtPlotPrintFilter::reset(QwtPlot *plot) const
{
    ...
    if (plot->titleLabel()) {
        ...
        if (title->text().testPaintAttribute(QwtText::PaintUsingTextColor)) {
            ...
            QPalette palette = title->palette();
            palette.setColor(QPalette::Active, Palette::Text, cache.titleColor);
            title->setPalette(palette);
        }
    ...
    if (plot->legend()) {
        ...
        for (QList<QWidget *>::iterator it = list.begin(); it != list.end(); ++it) {
            ...
            if (w->inherits("QwtLegendItem")) {
                QwtLegendItem *label = (QwtLegendItem *)w;
                const QwtPlotItem *plotItem = (const QwtPlotItem *)plot->legend()->find(label);
    ...
    for (int axis = 0; axis < QwtPlot::axisCnt; axis++) {
        ...
        if (scaleWidget) {
            QPalette palette = scaleWidget->palette();
            palette.setColor(QPalette::Active, Palette::WindowText, cache.scaleColor[axis]);
            scaleWidget->setPalette(palette);

...
private:
    ...
    QPointF intersectEdge(const QPointF &p1, const QPointF &p2, Edge edge) const;
...
public:
    ...
    QVector<QwtDoubleInterval> clipCircle(const QPointF &, double radius) const;

private:
    QList<QPointF> cuttingPoints(Edge, const QPointF &pos, double radius) const;
    double toAngle(const QPointF &, const QPointF &) const;
};

QwtPolygonClipper::QwtPolygonClipper(const QRect &r) : QRect(r) { }

inline void QwtPolygonClipper::addPoint(QPolygon &pa, uint pos, const QPoint &point) const
{
    if (uint(pa.size()) <= pos)
        pa.resize(pos + 5);
...
QPoint QwtPolygonClipper::intersectEdge(const QPoint &p1, const QPoint &p2, Edge edge) const
{
    int x = 0, y = 0;
    double m = 0;
...
void QwtPolygonClipper::clipEdge(Edge edge, const QPolygon &pa, QPolygon &cpa) const
{
    if (pa.count() == 0) {
        cpa.resize(0);
        return;
...
QwtPolygonClipperF::QwtPolygonClipperF(const QRectF &r) : QRectF(r) { }
...
QPointF QwtPolygonClipperF::intersectEdge(const QPointF &p1, const QPointF &p2, Edge edge) const
{
    double x = 0.0, y = 0.0;
    double m = 0;
...
void QwtPolygonClipperF::clipEdge(Edge edge, const QPolygonF &pa, QPolygonF &cpa) const
{
    if (pa.count() == 0) {
        cpa.resize(0);
        return;
...
QwtCircleClipper::QwtCircleClipper(const QRectF &r) : QRectF(r) { }

QVector<QwtDoubleInterval> QwtCircleClipper::clipCircle(const QPointF &pos, double radius) const
{
    ...
    if (points.size() <= 0) {
        ...
        const int in = contains(qwtPolar2Pos(pos, radius, angles[0] + (angles[1] - angles[0]) / 2));
        if (in) {
...
QList<QPointF> QwtCircleClipper::cuttingPoints(Edge edge, const QPointF &pos, double radius) const
{
    QList<QPointF> points;
...
*/
QPolygon QwtClipper::clipPolygon(const QRect &clipRect, const QPolygon &polygon)
{
...
/*!
   ...
   \return Clipped polygon
*/
QPolygonF QwtClipper::clipPolygonF(const QRectF &clipRect, const QPolygonF &polygon)
{
...
*/
QVector<QwtDoubleInterval> QwtClipper::clipCircle(const QRectF &clipRect, const QPointF &center,
                                                  double radius)
{

...
void QwtScaleWidget::layoutScale(bool update_geometry)
{
    ...
    d_data->titleOffset = d_data->margin + d_data->spacing + colorBarWidth
...
void QwtScaleWidget::drawColorBar(QPainter *painter, const QRect &rect) const
{
    ...
    QwtPainter::drawColorBar(painter, *d_data->colorBar.colorMap,
                             d_data->colorBar.interval.normalized(), sd->map(), sd->orientation(),
                             rect);
}
                               ...
                               const QRect &rect) const
{
    QRect r;
    double angle;
    int flags = d_data->title.renderFlags() & ~(Qt::AlignTop | Qt::AlignBottom | Qt::AlignVCenter);
...
QSize QwtScaleWidget::minimumSizeHint() const
{
    ...
    length += d_data->scaleDraw->minLength(QPen(Qt::black, d_data->penWidth), font());
...
int QwtScaleWidget::dimForLength(int length, const QFont &scaleFont) const
{
    ...
    dim += d_data->scaleDraw->extent(QPen(Qt::black, d_data->penWidth), scaleFont);
...
void QwtScaleWidget::setColorMap(const QwtDoubleInterval &interval, const QwtColorMap &colorMap)
{

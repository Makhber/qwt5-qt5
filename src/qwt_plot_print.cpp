...
void QwtPlot::print(QPaintDevice &paintDev, const QwtPlotPrintFilter &pfilter) const
{
                    ...
                    const QwtPlotPrintFilter &pfilter) const
{
    ...
    if ((pfilter.options() & QwtPlotPrintFilter::PrintLegend) && legend() && !legend()->isEmpty()) {
    ...
    for (axisId = 0; axisId < QwtPlot::axisCnt; axisId++) {
        ...
        if (scaleWidget) {
            ...
            printScale(painter, axisId, startDist, endDist, baseDist,
                       plotLayout()->scaleRect(axisId));
        }
    }
     ...
     */
    QRect boundingRect(canvasRect.left() - 1, canvasRect.top() - 1, canvasRect.width() + 2,
                       canvasRect.height() + 2);
...
void QwtPlot::printTitle(QPainter *painter, const QRect &rect) const
{
    ...
    const QColor color = titleLabel()->palette().color(QPalette::Active, QPalette::Text);
...
void QwtPlot::printLegendItem(QPainter *painter, const QWidget *w, const QRect &rect) const
{
...
void QwtPlot::printScale(QPainter *painter, int axisId, int startDist, int endDist, int baseDist,
...
void QwtPlot::printCanvas(QPainter *painter, const QRect &boundingRect, const QRect &canvasRect,

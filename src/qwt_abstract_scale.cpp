...
/*!
  ...
  \sa setAutoScale()
*/
void QwtAbstractScale::setScale(const QwtDoubleInterval &interval, double stepSize)
{
...
{
    const QwtScaleDiv scaleDiv = d_data->scaleEngine->divideScale(vmin, vmax, d_data->maxMajor,
                                                                  d_data->maxMinor, stepSize);
...
{
    rescale(d_data->scaleDraw->scaleDiv().lowerBound(), d_data->scaleDraw->scaleDiv().upperBound(),
            d_data->stepSize);
}

...
QwtPlotPicker::QwtPlotPicker(QwtPlotCanvas *canvas) : QwtPicker(canvas), d_xAxis(-1), d_yAxis(-1)
{
    if (!canvas)
        return;
...
QwtPlotPicker::QwtPlotPicker(int xAxis, int yAxis, QwtPlotCanvas *canvas)
    : QwtPicker(canvas), d_xAxis(xAxis), d_yAxis(yAxis)
{
}
...
/*!
  ...
  \sa QwtPlot::autoReplot(), QwtPlot::replot(), QwtPlotPicker::scaleRect()
*/
QwtPlotPicker::QwtPlotPicker(int xAxis, int yAxis, int selectionFlags, RubberBand rubberBand,
                             DisplayMode trackerMode, QwtPlotCanvas *canvas)
    : QwtPicker(selectionFlags, rubberBand, trackerMode, canvas), d_xAxis(xAxis), d_yAxis(yAxis)
{
}
...
QPoint QwtPlotPicker::transform(const QPointF &pos) const
{
    ...
    return QPoint(xMap.transform(pos.x()), yMap.transform(pos.y()));
}

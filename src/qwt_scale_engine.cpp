...
*/
int QwtScaleArithmetic::compareEps(double value1, double value2, double intervalSize)
{
...
*/
bool QwtScaleEngine::contains(const QwtDoubleInterval &interval, double value) const
{
    if (!interval.isValid())
        return false;

    if (QwtScaleArithmetic::compareEps(value, interval.minValue(), interval.width()) < 0) {
        return false;
    }

    if (QwtScaleArithmetic::compareEps(value, interval.maxValue(), interval.width()) > 0) {
        return false;
    }
...
*/
QwtScaleDiv QwtLinearScaleEngine::divideScale(double x1, double x2, int maxMajSteps,
...
*/
void QwtLinearScaleEngine::buildTicks(const QwtDoubleInterval &interval, double stepSize,
                                      int maxMinSteps,
...
*/
QList<double> QwtLinearScaleEngine::buildMajorTicks(const QwtDoubleInterval &interval,
                                                    double stepSize) const
{
...
*/
void QwtLinearScaleEngine::buildMinorTicks(const QList<double> &majorTicks, int maxMinSteps,
                                           ...
                                           QList<double> &mediumTicks) const
{
    ...
    if (QwtScaleArithmetic::compareEps((numTicks + 1) * qwtAbs(minStep), qwtAbs(stepSize), stepSize)
        > 0) {
        numTicks = 1;
...
*/
void QwtLog10ScaleEngine::autoScale(int maxNumSteps, double &x1, double &x2, double &stepSize) const
{
    if (x1 > x2)
        qSwap(x1, x2);

    QwtDoubleInterval interval(x1 / pow(10.0, lowerMargin()), x2 * pow(10.0, upperMargin()));
    ...
    if (testAttribute(QwtScaleEngine::Symmetric)) {
        const double delta = qwtMax(interval.maxValue() / logRef, logRef / interval.minValue());
...
*/
QwtScaleDiv QwtLog10ScaleEngine::divideScale(double x1, double x2, int maxMajSteps, int maxMinSteps,
                                             double stepSize) const
{
    QwtDoubleInterval interval = QwtDoubleInterval(x1, x2).normalized();
    interval = interval.limited(LOG_MIN, LOG_MAX);
...
/*!
   ...
   \sa buildMajorTicks(), buildMinorTicks
*/
void QwtLog10ScaleEngine::buildTicks(const QwtDoubleInterval &interval, double stepSize,
                                     int maxMinSteps,
                                     QList<double> ticks[QwtScaleDiv::NTickTypes]) const
{
    const QwtDoubleInterval boundingInterval = align(interval, stepSize);
...
QList<double> QwtLog10ScaleEngine::buildMajorTicks(const QwtDoubleInterval &interval,
                                                   double stepSize) const
{
...
QList<double> QwtLog10ScaleEngine::buildMinorTicks(const QList<double> &majorTicks, int maxMinSteps,
                                                   double stepSize) const
{
...
{
    return QwtDoubleInterval(::log10(interval.minValue()), ::log10(interval.maxValue()));
}

...
*/
void QwtPlotScaleItem::draw(QPainter *painter, const QwtScaleMap &xMap, const QwtScaleMap &yMap,
...
void QwtPlotScaleItem::updateScaleDiv(const QwtScaleDiv &xScaleDiv, const QwtScaleDiv &yScaleDiv)
{
    ...
    if (d_data->scaleDivFromAxis && sd) {
        sd->setScaleDiv(sd->orientation() == Qt::Horizontal ? xScaleDiv : yScaleDiv);
        updateBorders();
    }
}

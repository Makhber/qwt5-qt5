...
*/
QwtPlotRescaler::QwtPlotRescaler(QwtPlotCanvas *canvas, int referenceAxis, RescalePolicy policy)
    : QObject(canvas)
{
...
void QwtPlotRescaler::setIntervalHint(int axis, const QwtDoubleInterval &interval)
{
    if (axis >= 0 && axis < QwtPlot::axisCnt)
...
*/
QwtDoubleInterval QwtPlotRescaler::expandScale(int axis, const QSize &oldSize,
                                               const QSize &newSize) const
{
    ...
    case Expanding: {
        if (!oldSize.isEmpty()) {
            ...
            expanded = expandInterval(oldInterval, width, expandingDirection(axis));
        }
        break;
    }
    case Fitting: {
        ...
        if (dist > 0.0) {
            ...
            expanded = expandInterval(intervalHint(axis), width, expandingDirection(axis));
        }
        break;
    }
    }
...
*/
QwtDoubleInterval QwtPlotRescaler::syncScale(int axis, const QwtDoubleInterval &reference,
...
*/
QwtDoubleInterval QwtPlotRescaler::expandInterval(const QwtDoubleInterval &interval, double width,
...
void QwtPlotRescaler::updateScales(QwtDoubleInterval intervals[QwtPlot::axisCnt]) const
{
    ...
    for (int axis = 0; axis < QwtPlot::axisCnt; axis++) {
        if (axis == referenceAxis() || aspectRatio(axis) > 0.0) {
            ...
            if (plt->axisScaleDiv(axis)->lowerBound() > plt->axisScaleDiv(axis)->upperBound()) {
                qSwap(v1, v2);
            }

...
QRect QwtArrowButton::labelRect() const
{
    ...
    if (isDown()) {
        ...
        ph = style()->pixelMetric(QStyle::PM_ButtonShiftHorizontal, &option, this);
        pv = style()->pixelMetric(QStyle::PM_ButtonShiftVertical, &option, this);
        r.translate(ph, pv);
    }
...
{
    const bool isVertical = d_data->arrowType == Qt::UpArrow || d_data->arrowType == Qt::DownArrow;
    ...
    if (d_data->arrowType == Qt::LeftArrow || d_data->arrowType == Qt::RightArrow) {
        contentsSize.setWidth(d_data->num * arrow.width() + (d_data->num - 1) * Spacing);
        contentsSize.setHeight(arrow.height());
    } else {
        contentsSize.setWidth(arrow.width());
        contentsSize.setHeight(d_data->num * arrow.height() + (d_data->num - 1) * Spacing);
    }
...
QSize QwtArrowButton::minimumSizeHint() const
{
    ...
    QSize sz(2 * Margin + (MaxNum - 1) * Spacing + MaxNum * asz.width(), 2 * Margin + asz.height());

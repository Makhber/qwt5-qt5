...
int QwtDynGridLayout::maxRowWidth(int numCols) const
{
    ...
    for (uint index = 0; index < (uint)d_data->itemSizeHints.count(); index++) {
        col = index % numCols;
        colWidth[col] = qwtMax(colWidth[col], d_data->itemSizeHints[int(index)].width());
    }
...
QList<QRect> QwtDynGridLayout::layoutItems(const QRect &rect, uint numCols) const
{
    ...
    for (int i = 0; i < itemCount; i++) {
        ...
        QRect itemGeometry(colX[col], rowY[row], colWidth[col], rowHeight[row]);
...
void QwtDynGridLayout::layoutGrid(uint numCols, QVector<int> &rowHeight,
                                  QVector<int> &colWidth) const
{
    ...
    for (uint index = 0; index < (uint)d_data->itemSizeHints.count(); index++) {
        ...
        rowHeight[row] = (col == 0) ? size.height() : qwtMax(rowHeight[row], size.height());
        colWidth[col] = (row == 0) ? size.width() : qwtMax(colWidth[col], size.width());
    }
}
...
*/
void QwtDynGridLayout::stretchGrid(const QRect &rect, uint numCols, QVector<int> &rowHeight,
                                   QVector<int> &colWidth) const
{
    if (numCols == 0 || isEmpty())
        return;

...
*/
QwtKnob::QwtKnob(QWidget *parent) : QwtAbstractSlider(Qt::Horizontal, parent)
{
    initKnob();
}
...
void QwtKnob::setTotalAngle(double angle)
{
    ...
    scaleDraw()->setAngleRange(-0.5 * d_data->totalAngle, 0.5 * d_data->totalAngle);
    layoutKnob();
}
...
void QwtKnob::drawKnob(QPainter *painter, const QRect &r)
{
    ...
    const QRect aRect(r.center().x() - radius, r.center().y() - radius, 2 * radius, 2 * radius);
...
double QwtKnob::getValue(const QPoint &p)
{
    ...
    double newValue = 0.5 * (minValue() + maxValue())
            + (arc + d_data->nTurns * 360.0) * (maxValue() - minValue()) / d_data->totalAngle;
...
void QwtKnob::drawMarker(QPainter *p, double arc, const QColor &c)
{
    ...
    case Dot: {
        ...
        p->drawEllipse(xm - qRound(sa * rb) - d_data->dotWidth / 2,
                       ym - qRound(ca * rb) - d_data->dotWidth / 2, d_data->dotWidth,
                       d_data->dotWidth);
        break;
    }
    case Line: {
        ...
        p->drawLine(xm - qRound(sa * rb), ym - qRound(ca * rb), xm - qRound(sa * re),
                    ym - qRound(ca * re));
...
void QwtKnob::recalcAngle()
{
    ...
    if (maxValue() == minValue()) {
        ...
        d_data->angle = (value() - 0.5 * (minValue() + maxValue())) / (maxValue() - minValue())
                * d_data->totalAngle;

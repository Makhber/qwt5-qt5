...
private:
    ...
    inline const QwtTextEngine *engine(EngineMap::const_iterator &it) const { return it.value(); }
...
void QwtTextEngineDict::setTextEngine(QwtText::TextFormat format, QwtTextEngine *engine)
{
    if (format == QwtText::AutoText)
        return;
...
{
    return d_data->renderFlags == other.d_data->renderFlags && d_data->text == other.d_data->text
            && d_data->font == other.d_data->font && d_data->color == other.d_data->color
...
int QwtText::heightForWidth(int width, const QFont &defaultFont) const
{
    ...
    if (d_data->layoutAttributes & MinimumLayout) {
        ...
        h = d_data->textEngine->heightForWidth(font, d_data->renderFlags, d_data->text,
                                               width + left + right);

        h -= top + bottom;
    } else {
        h = d_data->textEngine->heightForWidth(font, d_data->renderFlags, d_data->text, width);
    }
...
void QwtText::draw(QPainter *painter, const QRect &rect) const
{
    ...
    d_data->textEngine->draw(painter, expandedRect, d_data->renderFlags, d_data->text);

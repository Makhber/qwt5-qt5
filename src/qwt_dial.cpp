...
*/
QwtDialScaleDraw::QwtDialScaleDraw(QwtDial *parent) : d_parent(parent), d_penWidth(1) { }
...
QwtDial::QwtDial(QWidget *parent) : QwtAbstractSlider(Qt::Horizontal, parent)
{
    initDial();
}
...
QRect QwtDial::contentsRect() const
{
    ...
    if (lw > 0) {
        r.setRect(r.x() + lw, r.y() + lw, r.width() - 2 * lw, r.height() - 2 * lw);
    }
    return r;
}
...
QRect QwtDial::scaleContentsRect() const
{
    ...
    return QRect(rect.x() + scaleDist, rect.y() + scaleDist, rect.width() - 2 * scaleDist,
                 rect.height() - 2 * scaleDist);
}
...
void QwtDial::drawFrame(QPainter *painter)
{
    ...
    QRect r = boundingRect();
    r.setRect(r.x() + lw / 2 - off, r.y() + lw / 2 - off, r.width() - lw + off + 1,
              r.height() - lw + off + 1);
    r.setX(r.x() + 1);
    r.setY(r.y() + 1);
...
/*!
  ...
  \param cg ColorGroup
*/
void QwtDial::drawNeedle(QPainter *painter, const QPoint &center, int radius, double direction,
...
*/
void QwtDial::drawScale(QPainter *painter, const QPoint &center, int radius, double origin,
                        double minArc, double maxArc) const
{
...
void QwtDial::drawScaleContents(QPainter *, const QPoint &, int) const
{
...
void QwtDial::updateScale()
{
    if (d_data->scaleDraw) {
        ...
        const QwtScaleDiv scaleDiv = scaleEngine.divideScale(
                minValue(), maxValue(), d_data->maxMajIntv, d_data->maxMinIntv, d_data->scaleStep);
...
void QwtDial::setScaleOptions(int options)
{
    ...
    sd->enableComponent(QwtAbstractScaleDraw::Backbone, options & ScaleBackbone);

    sd->enableComponent(QwtAbstractScaleDraw::Ticks, options & ScaleTicks);

    sd->enableComponent(QwtAbstractScaleDraw::Labels, options & ScaleLabel);
}
...
*/
void QwtDial::setScaleTicks(int minLen, int medLen, int majLen, int penWidth)
{
    QwtDialScaleDraw *sd = d_data->scaleDraw;
    if (sd) {
...
double QwtDial::getValue(const QPoint &pos)
{
    ...
    const double completeCircle =
            360.0 / (d_data->maxScaleArc - d_data->minScaleArc) * (maxValue() - minValue());
        ...
        if (d_data->previousDir >= 0.0) // valid direction
        {
            ...
            if (clockWise) {
                ...
                if (wrapping()) {
                    ...
                    if (posValue - mouseOffset() > maxValue() || value() == maxValue()) {
                ...
                if (wrapping()) {
                    ...
                    if (posValue - mouseOffset() < minValue() || value() == minValue()) {

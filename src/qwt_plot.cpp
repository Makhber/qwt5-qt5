    ...
    for (int axisId = 0; axisId < axisCnt; axisId++) {
        if (axisEnabled(axisId)) {
            ...
            if (axisId == yLeft || axisId == yRight) {
                int hDiff = (majCnt - 1) * niceDist - scaleWidget->minimumSizeHint().height();
                if (hDiff > dh)
                    dh = hDiff;
            } else {
                int wDiff = (majCnt - 1) * niceDist - scaleWidget->minimumSizeHint().width();
                if (wDiff > dw)
                    dw = wDiff;
            }
        }
...
void QwtPlot::updateLayout()
{
    ...
    if (d_data->legend && d_data->layout->legendPosition() != ExternalLegend) {
...
void QwtPlot::updateTabOrder()
{
    ...
    if (d_data->legend.isNull() || d_data->layout->legendPosition() == ExternalLegend
    ...
    const bool canvasFirst = d_data->layout->legendPosition() == QwtPlot::BottomLegend
    ...
    while ((w = w->nextInFocusChain()) != d_data->canvas) {
        ...
        if (w->focusPolicy() != NoFocus && w->parent()
            && w->parent() == d_data->legend->contentsWidget()) {
            isLegendItem = true;
        }
...
void QwtPlot::drawCanvas(QPainter *painter)
{
    ...
    drawItems(painter, d_data->canvas->contentsRect(), maps, QwtPlotPrintFilter());
}
...
void QwtPlot::drawItems(QPainter *painter, const QRect &rect, const QwtScaleMap map[axisCnt],
                        const QwtPlotPrintFilter &pfilter) const
{
    ...
    for (QwtPlotItemIterator it = itmList.begin(); it != itmList.end(); ++it) {
        ...
        if (item && item->isVisible()) {
            ...
            item->draw(painter, map[item->xAxis()], map[item->yAxis()], rect);
...
QwtScaleMap QwtPlot::canvasMap(int axisId) const
{
    ...
    if (axisEnabled(axisId)) {
        ...
        if (axisId == yLeft || axisId == yRight) {
            map.setPaintInterval(canvasRect.bottom() - margin, canvasRect.top() + margin);
        } else {
            map.setPaintInterval(canvasRect.left() + margin, canvasRect.right() - margin);
        }
    }
    return map;
}

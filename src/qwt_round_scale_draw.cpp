...
{
public:
    PrivateData() : center(50, 50), radius(50), startAngle(-135 * 16), endAngle(135 * 16) { }
...
//! Copy constructor
QwtRoundScaleDraw::QwtRoundScaleDraw(const QwtRoundScaleDraw &other) : QwtAbstractScaleDraw(other)
{
...
void QwtRoundScaleDraw::drawLabel(QPainter *painter, double value) const
{
    ...
    if ((tval > d_data->startAngle + 359 * 16) || (tval < d_data->startAngle - 359 * 16)) {
        return;
    }
    ...
    const int x = d_data->center.x() + qRound((radius + sz.width() / 2.0) * sin(arc));
    const int y = d_data->center.y() - qRound((radius + sz.height() / 2.0) * cos(arc));
...
void QwtRoundScaleDraw::drawBackbone(QPainter *painter) const
{
    ...
    painter->drawArc(x, y, 2 * radius, 2 * radius, -a2, a2 - a1 + 1); // counterclockwise
}
...
int QwtRoundScaleDraw::extent(const QPen &pen, const QFont &font) const
{
    ...
    if (hasComponent(QwtAbstractScaleDraw::Labels)) {
        ...
        for (uint i = 0; i < (uint)ticks.count(); i++) {
            ...
            const int tval = map().transform(value);
            if ((tval < d_data->startAngle + 360 * 16) && (tval > d_data->startAngle - 360 * 16)) {
                const double arc = tval / 16.0 / 360.0 * 2 * M_PI;

...
void QwtPlotLayout::LayoutData::init(const QwtPlot *plot, const QRect &rect)
{
    ...
    if (plot->plotLayout()->legendPosition() != QwtPlot::ExternalLegend && plot->legend()) {
        legend.frameWidth = plot->legend()->frameWidth();
        legend.vScrollBarWidth = plot->legend()->verticalScrollBar()->sizeHint().width();
        legend.hScrollBarHeight = plot->legend()->horizontalScrollBar()->sizeHint().height();
    ...
    for (int axis = 0; axis < QwtPlot::axisCnt; axis++) {
        if (plot->axisEnabled(axis)) {
            ...
            if (scaleWidget->scaleDraw()->hasComponent(QwtAbstractScaleDraw::Ticks)) {
                scale[axis].tickOffset += (int)scaleWidget->scaleDraw()->majTickLength();
            }

            scale[axis].dimWithoutTitle =
                    scaleWidget->dimForLength(QWIDGETSIZE_MAX, scale[axis].scaleFont);

            if (!scaleWidget->title().isEmpty()) {
                scale[axis].dimWithoutTitle -= scaleWidget->titleHeightForWidth(QWIDGETSIZE_MAX);
            }
        } else {
...
{
public:
    PrivateData() : margin(0), spacing(5), alignCanvasToScales(false) { }
...
QSize QwtPlotLayout::minimumSizeHint(const QwtPlot *plot) const
{
    ...
    for (axis = 0; axis < QwtPlot::axisCnt; axis++) {
        ...
        canvasBorder[axis] = plot->canvas()->frameWidth() + d_data->canvasMargin[axis] + 1;
    }

    for (axis = 0; axis < QwtPlot::axisCnt; axis++) {
        ScaleData &sd = scaleData[axis];
        if (sd.w && (axis == QwtPlot::xBottom || axis == QwtPlot::xTop)) {
            if ((sd.minLeft > canvasBorder[QwtPlot::yLeft]) && scaleData[QwtPlot::yLeft].w) {
            ...
            if ((sd.minRight > canvasBorder[QwtPlot::yRight]) && scaleData[QwtPlot::yRight].w) {
        ...
        if (sd.h && (axis == QwtPlot::yLeft || axis == QwtPlot::yRight)) {
            if ((sd.minLeft > canvasBorder[QwtPlot::xBottom]) && scaleData[QwtPlot::xBottom].h) {
            ...
            if ((sd.minLeft > canvasBorder[QwtPlot::xTop]) && scaleData[QwtPlot::xTop].h) {
    ...
    if (title && !title->text().isEmpty()) {
        ...
        const bool centerOnCanvas =
                !(plot->axisEnabled(QwtPlot::yLeft) && plot->axisEnabled(QwtPlot::yRight));

        int titleW = w;
        if (centerOnCanvas) {
            titleW -= scaleData[QwtPlot::yLeft].w + scaleData[QwtPlot::yRight].w;
        }
    ...
    if (d_data->legendPos != QwtPlot::ExternalLegend && legend && !legend->isEmpty()) {
        if (d_data->legendPos == QwtPlot::LeftLegend || d_data->legendPos == QwtPlot::RightLegend) {
...
*/
QRect QwtPlotLayout::alignLegend(const QRect &canvasRect, const QRect &legendRect) const
{
    QRect alignedRect = legendRect;

    if (d_data->legendPos == QwtPlot::BottomLegend || d_data->legendPos == QwtPlot::TopLegend) {
...
/*!
  ...
  \sa Options
*/
void QwtPlotLayout::expandLineBreaks(int options, const QRect &rect, int &dimTitle,
                                     int dimAxis[QwtPlot::axisCnt]) const
{
    dimTitle = 0;
    for (int axis = 0; axis < QwtPlot::axisCnt; axis++)
        dimAxis[axis] = 0;
    ...
    while (!done) {
        ...
        for (int axis = 0; axis < QwtPlot::axisCnt; axis++) {
            const struct LayoutData::t_scaleData &scaleData = d_data->layoutData.scale[axis];
            ...
            if (scaleData.isEnabled) {
                ...
                } else // QwtPlot::yLeft, QwtPlot::yRight
                {
                    ...
                    if (dimAxis[QwtPlot::xBottom] > 0) {
                        length += qwtMin(d_data->layoutData.scale[QwtPlot::xBottom].tickOffset,
                                         scaleData.start - backboneOffset[QwtPlot::xBottom]);
                    }
                    if (dimAxis[QwtPlot::xTop] > 0) {
                        length += qwtMin(d_data->layoutData.scale[QwtPlot::xTop].tickOffset,
...
void QwtPlotLayout::alignScales(int options, QRect &canvasRect,
                                QRect scaleRect[QwtPlot::axisCnt]) const
{
    ...
    for (axis = 0; axis < QwtPlot::axisCnt; axis++) {
        ...
        if (axis == QwtPlot::xTop || axis == QwtPlot::xBottom) {
            ...
            if (scaleRect[QwtPlot::yLeft].isValid()) {
                ...
                if (d_data->alignCanvasToScales && leftOffset < 0) {
                    canvasRect.setLeft(qwtMax(canvasRect.left(), axisRect.left() - leftOffset));
                } else {
                    if (leftOffset > 0)
            ...
            const int rightOffset = backboneOffset[QwtPlot::yRight] - endDist + 1;
            ...
            if (scaleRect[QwtPlot::yRight].isValid()) {
                ...
                if (d_data->alignCanvasToScales && rightOffset < 0) {
                    canvasRect.setRight(qwtMin(canvasRect.right(), axisRect.right() + rightOffset));
                } else {
                    if (rightOffset > 0)
        ...
        {
            const int bottomOffset = backboneOffset[QwtPlot::xBottom] - endDist + 1;
            ...
            if (scaleRect[QwtPlot::xBottom].isValid()) {
                ...
                if (d_data->alignCanvasToScales && bottomOffset < 0) {
                    canvasRect.setBottom(
                            qwtMin(canvasRect.bottom(), axisRect.bottom() + bottomOffset));
                } else {
                    if (bottomOffset > 0)
            ...
            if (scaleRect[QwtPlot::xTop].isValid()) {
                ...
                if (d_data->alignCanvasToScales && topOffset < 0) {
                    canvasRect.setTop(qwtMax(canvasRect.top(), axisRect.top() - topOffset));
                } else {
                    if (topOffset > 0)
    ...
    if (d_data->alignCanvasToScales) {
        ...
        if (scaleRect[QwtPlot::xBottom].isValid() && scaleRect[QwtPlot::xTop].isValid()) {
        ...
        if (scaleRect[QwtPlot::yLeft].isValid() && scaleRect[QwtPlot::yRight].isValid()) {
...
*/
void QwtPlotLayout::activate(const QwtPlot *plot, const QRect &plotRect, int options)
{
    invalidate();
    ...
    if (!(options & IgnoreMargin)) {
        ...
        rect.setRect(rect.x() + d_data->margin, rect.y() + d_data->margin,
                     rect.width() - 2 * d_data->margin, rect.height() - 2 * d_data->margin);
    }
    ...
    if (!(options & IgnoreLegend) && d_data->legendPos != QwtPlot::ExternalLegend && plot->legend()
        && !plot->legend()->isEmpty()) {
        ...
        if (d_data->layoutData.legend.frameWidth && !(options & IgnoreFrames)) {
    ...
    */
    ...
    if (dimTitle > 0) {
        d_data->titleRect = QRect(rect.x(), rect.y(), rect.width(), dimTitle);
        ...
        if (d_data->layoutData.scale[QwtPlot::yLeft].isEnabled
            ...
            d_data->titleRect.setWidth(rect.width() - dimAxes[QwtPlot::yLeft]
    ...
    d_data->canvasRect.setRect(rect.x() + dimAxes[QwtPlot::yLeft],

...
*/
QwtLegendItem::QwtLegendItem(QWidget *parent) : QwtTextLabel(parent)
{
...
/*!
  ...
  \param parent Parent widget
*/
QwtLegendItem::QwtLegendItem(const QwtSymbol &symbol, const QPen &curvePen, const QwtText &text,
                             QWidget *parent)
    : QwtTextLabel(parent)
{
    d_data = new PrivateData;
...
void QwtLegendItem::drawIdentifier(QPainter *painter, const QRect &rect) const
{
    ...
    if ((d_data->identifierMode & ShowLine) && (d_data->curvePen.style() != Qt::NoPen)) {
        ...
        QwtPainter::drawLine(painter, rect.left(), rect.center().y(), rect.right(),
                             rect.center().y());
        painter->restore();
    }

    if ((d_data->identifierMode & ShowSymbol) && (d_data->symbol->style() != QwtSymbol::NoSymbol)) {
        QSize symbolSize = QwtPainter::metricsMap().screenToLayout(d_data->symbol->size());

        // scale the symbol size down if it doesn't fit into rect.

        if (rect.width() < symbolSize.width()) {
            const double ratio = double(symbolSize.width()) / double(rect.width());
...
void QwtLegendItem::drawItem(QPainter *painter, const QRect &rect) const
{
    ...
    const QRect identifierRect(rect.x() + m, rect.y(), identifierWidth, rect.height());

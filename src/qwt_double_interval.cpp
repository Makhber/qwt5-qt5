...
QwtDoubleInterval QwtDoubleInterval::symmetrize(double value) const
{
    ...
    const double delta = qwtMax(qwtAbs(value - d_maxValue), qwtAbs(value - d_minValue));
...
*/
QwtDoubleInterval QwtDoubleInterval::limited(double lowerBound, double upperBound) const
{
...
QwtDoubleInterval QwtDoubleInterval::extend(double value) const
{
    ...
    return QwtDoubleInterval(qwtMin(value, d_minValue), qwtMax(value, d_maxValue), d_borderFlags);
}
